...
var import_node_os, import_node_path, import_node_util, import_core, import_vendor_core;
var init_repl = __esm({
  "src/repl.ts"() {
    "use strict";
    ...
    import_core = require("./core.cjs");
    import_vendor_core = require("./vendor-core.cjs");
  }
});
...
function main() {
  return __async(this, null, function* () {
    ...
    if (argv.env) {
      const envPath = import_index.path.resolve((_a = import_index.$.cwd) != null ? _a : process.cwd(), argv.env);
      import_index.$.env = (0, import_util.readEnvFromFile)(envPath, process.env);
    }
    ...
    if (argv.repl) {
      yield (yield Promise.resolve().then(() => (init_repl(), repl_exports))).startRepl();
      return;
    }
    if (argv.eval) {
...
function writeAndImport(_0, _1) {
  return __async(this, arguments, function* (script, filepath, origin = filepath) {
    ...
    try {
      process.once("exit", () => import_index.fs.rmSync(filepath, { force: true }));
...
function importPath(_0) {
  return __async(this, arguments, function* (filepath, origin = filepath) {
    ...
    if (argv.install) {
      ...
      yield (0, import_deps.installDeps)(deps, dir, argv.registry);
    }
